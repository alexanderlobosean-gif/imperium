-- Script FINAL para AdminBanking
-- Remove ON CONFLICT e usa WHERE NOT EXISTS
...
-- 2. Criar Ana Oliveira - método sem ON CONFLICT
...
SELECT 
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM auth.users WHERE email = 'ana.oliveira@imperium.test'
);

-- Depois criar em profiles usando o ID gerado
...
SELECT 
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM profiles WHERE email = 'ana.oliveira@imperium.test'
);

-- 3. Criar Carlos Pereira - método sem ON CONFLICT
-- Primeiro criar em auth.users
INSERT INTO auth.users (
    id,
    email,
...
SELECT 
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM auth.users WHERE email = 'carlos.pereira@imperium.test'
);
...
SELECT 
    ...
    NOW()
WHERE NOT EXISTS (
    SELECT 1 FROM profiles WHERE email = 'carlos.pereira@imperium.test'
);
